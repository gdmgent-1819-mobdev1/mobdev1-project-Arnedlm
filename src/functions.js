...
function addArraytoDatabase(array){
  array.forEach((elem) => {
    ...
    let kot = new classes.Kot("€ " + elem.Huurprijs, "€ " + elem.Waarborg, elem.Type, elem.Oppervlakte + " m²", elem.Verdieping, elem["Privé toilet"], elem["Privé douche"], elem["Privé keuken"], elem.Opties, elem.Straat + " " + elem.Huisummer, elem.Plaats);
...
export function showUserInfo(userInfo){
  firebase.auth().onAuthStateChanged(function(user) {
    if(user){
      userInfo.innerHTML = user.email + "(<a id='logout' href='' >Uitloggen</a>)"
...
export function addMarkers(mapboxgl, map){
  arrayCoordinates.forEach((elem) => {
    let popup = new mapboxgl.Popup({ offset: 25 })
      .setHTML('<a href="/detail?id=' + elem.id + '" data-navigo>Meer info</a>');
...
export function addKot(){
  ...
  let kot = new classes.Kot("€ " + huurprijs, "€ " + waarborg, type, opp + " m²", verdieping, toilet, douche, keuken, meubels, straat, plaats);
